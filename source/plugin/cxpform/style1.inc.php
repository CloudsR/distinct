...
if($forminfo['form_type'] == '2' && $op == 'search'){
	...
	include template('cxpform:data_style1');	
}else{
	...
	if(count($forminfo) > 0){
		...
		if(!submitcheck('formsubmit')) {
			...
			// if($forminfo['form_type'] == '2' || $op == 'search1'){
			if($forminfo['form_type'] == '2'){
				...
				$page = $page > 0 ? $page : 1;
				$start = !$pages ? 0 : ($page - 1) * $limit;
				$url = 'plugin.php?id=cxpform:style1&form_id=' . $form_id;
				$condstr = ' where A.form_id=' . $form_id;
				...
				// var_dump($submit_setting['allowshowall']);
				if(!$submit_setting['allowshowall']){
					$count = 0;
					$results = array();
				}
				...
				include template('cxpform:style1');
			}
		}else{
			// 插到cxpform_contents
			...
			// 帐号限制
			if($_G['uid'] > 0){
				$user_today_count = DB::result_first("select count(*) from " . DB::table('cxpform_contents') . " where form_id='" . $form_id . "' and user_id='" . $_G['uid'] . "' and addtime>='" . dmktime(date('Y-m-d 00:00:00')) . "'");
			...
			// IP次数限制
			$ip_today_count = DB::result_first("select count(*) from " . DB::table('cxpform_contents') . " where form_id='" . $form_id . "' and ip='" . $_G['clientip'] . "' and addtime>='" . dmktime(date('Y-m-d 00:00:00')) . "'");
